...
dir_path = '../../datasets/human_activities_videos/';

video_files = dir([dir_path '*.avi']);



for file_n = 1:size(video_files);

    video_path = [dir_path video_files(file_n).name];
    
    [pa,fi,ex] = fileparts(video_files(file_n).name);
    
    boxes_path = [dir_path fi 'boxes.mat'];
    
    tracking = cell(1);
    
    obj = setupSystemObjects(video_path, boxes_path);
    ...
    for f_n=1:(obj.reader.Duration*obj.reader.FrameRate)
        if (f_n/obj.reader.FrameRate >= obj.reader.Duration)
           break; 
        end
        obj.reader.CurrentTime = f_n/obj.reader.FrameRate;
    ...
    save([dir_path fi 'matlabtrack.mat'],'tracking');
end
...
% foreground objects, and displaying results.
    ...
    function obj = setupSystemObjects(video_path,boxes_path)
        % Initialize Video I/O
        % Create objects for reading a video from a file, drawing the tracked
        % objects in each frame, and playing the video.
        
        % Create a video file reader.
        obj.reader = VideoReader(video_path);
