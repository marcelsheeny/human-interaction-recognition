...
for set=1:size(sets,1)
    ...
    for j=1:size(videos,1)      
        file_images = dir([path sets(set).name '/' videos(j).name '/rgb*']);
        ...
        tracked_bboxes = cell(1);

        % Detect moving objects, and track them across video frames.
        for f_n=1:length(file_images)
...
%                frame = obj.images{f_n};
            ...
            tracked_bboxes{f_n} = cell(1);

            if (length(tracks) == 1)
                tracked_bboxes{f_n}{1}= tracks(1,1);
                tracked_bboxes{f_n}{2}= tracks(1,1);
                disp('lenght 1');
            elseif (length(tracks) == 2)
                if (tracks(1,1).bbox(1,1) < tracks(1,2).bbox(1,1))
                    tracked_bboxes{f_n}{1}= tracks(1,1);
                    tracked_bboxes{f_n}{2}= tracks(1,2);
                else
                    tracked_bboxes{f_n}{2}= tracks(1,1);
                    tracked_bboxes{f_n}{1}= tracks(1,2);
                end

            elseif (isempty(tracks))
                if (f_n-1 > 0)
                    tracked_bboxes{f_n}{1} = tracked_bboxes{f_n-1}{1};
                    tracked_bboxes{f_n}{2} = tracked_bboxes{f_n-1}{2};
                else
                    tracked_bboxes{f_n}{1} = [1 1 200 200];
                    tracked_bboxes{f_n}{2} = [1 1 200 200];
                end
                disp('lenght 0');
            else
                disp(['lenght 3?' num2str(length(tracks))]);
                if (tracks(1,1).bbox(1,1) < tracks(1,2).bbox(1,1))
                    tracked_bboxes{f_n}{1}= tracks(1,1);
                    tracked_bboxes{f_n}{2}= tracks(1,2);
                else
            ...
            %for p_n = 1:length(tracks)
            %    if
            %    tracked_bboxes{f_n}{tracks(1,p_n).id}= tracks(1,p_n);
            %end
            
        end
        annotation.tracked_bboxes = tracked_bboxes;
    %{
        images = cell(1);
    
        for image_idx = 1:length(file_images)
            images{image_idx} = imread([path sets(set).name '/' videos(j).name '/' file_images(image_idx).name]);
...
% foreground objects, and displaying results.
    ...
    function obj = setupSystemObjects(images,boxes_path)
        ...
        %    'MinimumBlobArea', 400);
        temp = load(boxes_path);
        
        annotation = temp.annotation;
        
        obj.bboxes = temp.annotation.bboxes;
        clear temp;
    end
...
% remove noisy pixels and to fill the holes in the remaining blobs.  
    ...
    function [centroids, bboxes] = detectObjects(obj,frame_n)
        ...
        bboxes = int32(round([obj.bboxes{frame_n}(:,1:2) obj.bboxes{frame_n}(:,3:4) - obj.bboxes{frame_n}(:,1:2)]));
