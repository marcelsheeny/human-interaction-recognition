...
function [joints, heatmaps] = applyNetImage(im, net, opt)
...
if (opt.process_heatmap)
    joints = getJointsFromHeatmaps(heatmaps,opt);
end
disp(toc); 
...
if opt.visualise
    ...
    for i=1:opt.numJoints
        ...
        %max_val = max(max(heatmaps(:,:,i)));
        %min_val = min(min(heatmaps(:,:,i)));

        %dif = max_val - min_val;

        %thres = max_val - (dif*0.3);
        
        %heatmaps(:,:,i) = imcomplement(heatmaps(:,:,i).*(heatmaps(:,:,i) > thres));
